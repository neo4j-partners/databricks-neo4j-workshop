...
**Last status pass: 2026-08-09, against `HEAD` at `5fb3097` plus 14 modified files.** Since the previous pass: the Lab 6 `CALL {}` blocker is fixed, the resource list moved into one shared `build_resources` in `agent.py` and grew from four entries to twelve, Lab 6 now registers into the same UC model Lab 5 created rather than logging an unregistered run, and two end-to-end job runs are in flight right now, one per lab. The pass before that landed `b82ca4b` and `5fb3097`, closing 9.1; rebuilt the lakehouse gold tables; fixed defect E; and closed 16 of 24 documentation findings.
...
- **Defect F is open and it is nondeterminism, not a wrong answer.** The components-for-an-aircraft question returned 0 rows then 25 rows on two consecutive runs against an identical graph, because the model sometimes chains `AFFECTS_SYSTEM` then `HAS_COMPONENT` instead of using `HAS_EVENT`. See section 5. IN FLIGHT.
...
- **Redeploy with `DatabricksSQLWarehouse` and the gold `DatabricksTable` entries declared, then put one Genie question through the endpoint.** **This is Phase 2's completion criterion and the only unmet one.** The declaration is written and shared, `build_resources` in `agent.py`. **RUNNING**, job `640691439045913`. Section 5 question 2.
...
Fourteen, numbered. Each says what the decision is, what the evidence is, and what it blocks. **Four are IN FLIGHT**, meaning an agent is working them right now, so they are not waiting on anybody.

### In flight

1. **Lab 5 has no notebook 02 in the participant path.** `01_langgraph_agent.ipynb` ends promising one that logs and deploys `agent.py`, and Lab 6 lists the deployed endpoint as a prerequisite. **So Lab 6 cannot start from a clean Lab 5.** Blocks the Phase 4 dry run and Lab 6's own entry condition. **IN FLIGHT.**
2. **`genie_node` is unauthorized from the serving principal.** "is not authorized to use or monitor this SQL Endpoint", `auth_type=model-serving`. Declaring `DatabricksGenieSpace` at log time does not carry the SQL warehouse behind the space; `DatabricksSQLWarehouse` and the gold `DatabricksTable` entries have to be declared beside it. **Every participant who deploys Lab 5 hits this.** Blocks Phase 2's completion criterion and every Genie number in the memory-off baseline. **RUNNING**, job `640691439045913`, against a twelve-entry `build_resources`.
3. **The gold-table Genie edit to `lab/workshop.py`.** Settled as do-not-attach. `sensor_health.health_status` can never emit `ANOMALY`, because the rule fires on `p95 > avg + 2*stddev` while `p95` is approximately `avg + 1.645*stddev`. Measured 284 WARNING, 4 NORMAL, 0 ANOMALY across 288 sensors. The edit removes two `TABLE_COMMENTS` and two `COLUMN_COMMENTS` entries. Blocks nothing downstream; the cost of leaving it is a comment that promises a value the table cannot produce. **IN FLIGHT.** Decision recorded in section 4.
4. **Defect F: the components-for-an-aircraft question is nondeterministic.** Two consecutive runs against an identical graph returned 0 rows, then 25 rows. The model sometimes chains `AFFECTS_SYSTEM` then `HAS_COMPONENT` instead of using `HAS_EVENT`. **A participant asking twice gets two answers, one of them "nothing found".** Blocks Phase 1 being called closed, since a passing suite that flips on rerun is not a measurement. **IN FLIGHT.**
...
2. Close defect F, the nondeterministic components-for-an-aircraft question. **A suite that flips on rerun is not a measurement.** IN FLIGHT.
...
**Retaken. 48 of 48 across 9 groups against `5fb3097`, defects A, B, C and E all fixed.** The record below stands as the reason it had to be retaken. What is left is defect F, the nondeterministic components question, which is section 5 question 4 and IN FLIGHT.
...
**Taken as recorded in section 4: do not attach, and drop the two `TABLE_COMMENTS` and two `COLUMN_COMMENTS` entries.** IN FLIGHT as section 5 question 3. The record below is why.
