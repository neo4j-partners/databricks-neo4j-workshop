...
- **It took two independent fixes, and both are lessons worth teaching.**
  ...
  - **The fix is one shared function,** `build_resources(genie_agent_id, warehouse_id)` at `agent.py:147`. Twelve entries: the Genie space, the SQL warehouse, the eight gold tables, and the supervisor and embedding endpoints. Both the Lab 5 deploy notebook and Lab 6 call it, so the two labs cannot drift.
...
- ~~**BLOCKING: the deployed endpoint does not run the code on disk.**~~ **DONE 2026-08-09. Version 12 logged, deployed, and verified from the artifact.**
  - **The endpoint had been serving UC model version 9, which carried `databricks-meta-llama-3-3-70b-instruct` and a `tools.py` 121 lines shorter than the repo's,** 1097 against 1218. Versions 10 and 11 were llama too. **No Claude version had ever been logged.** So the endpoint was stale on the model and on the agent code, independently, and any evaluation taken against it would have been filed as a Claude baseline while measuring llama on old code.
  - **Version 12 was verified by downloading the artifact and reading it, not by trusting the version number.** `code/data_utils.py:46` reads `databricks-claude-sonnet-5`, `code/tools.py` is 1218 lines matching the repo exactly, the MLmodel `llm_endpoint` config agrees, and the 12 resources name the Claude and BGE serving endpoints. The only `llama` string anywhere in the artifact is a docstring listing accepted models.
  - **Deployed to the existing endpoint** `fleet-ops-assistant-ryan-knight-neo4j-com`, READY, 100 percent traffic to version 12. **No second endpoint**, which matters because Lab 6 redeploys this one. No secret scope, credential or environment variable was changed.
  - **The one deviation from the notebook:** `mlflow.set_experiment` had to be called before `log_model` rather than before the deploy, because outside a notebook there is no implicit experiment and `start_run` fails with `Could not find experiment with ID None`. Same experiment name, so the run and version land where the notebook puts them.
  - **The backwards-arrow fix was already in version 9,** verified by reading both files. It predates the drift and was never part of it.
  - **Provenance rule this earned:** the deploy notebook run outside a notebook needs the experiment set earlier than the notebook sets it. Anyone reproducing this from a script hits the same error.
...
- **Re-measure routing on Claude.** The 48 of 48 figure and the deploy notebook's 6 of 6 routing score were both taken on llama. **A model change voids routing numbers exactly the way a prompt change does.** Two runs were owed; one is done.
  ...
  - ~~The deploy notebook's 6-pair evaluation.~~ **DONE 2026-08-09 against version 12 on the live endpoint. 5 of 6 pass. The anchor fails, four trials out of four.** Full report in `worklog/lab5-eval-v12.md`. All three tools were live on every call.
    - **Pairs 1 to 5 route exactly as expected and answer correctly.** `genie_node` for average EGT, `cypher_node` for hydraulic components and for the 23 maintenance events on N10004, `graphrag_node` for the EGT exceedance procedure (ENG-OVH-001 and ENG-OVH-003, both expected facts verbatim) and for the vibration decision tree.
    - **The anchor is unstable across trials, and no trial answered it correctly.** Two trials routed `['genie_node']` alone, scoring 0.33. Two routed all three tools, scoring 1.0. **The 1.0 trials were also wrong**, so routing score and answer quality came apart, which is worth knowing before anyone treats the routing number as a proxy for correctness.
    - **The routing defect: there is no decomposition anywhere in the design.** `tools.py:838` is `question = state["question"]`, so the supervisor forwards the entire multi-part question to each tool. The anchor's first clause is a reading question, so `CYPHER_PROMPT` at `tools.py:408` correctly fires its refusal and returns `RETURN 'The graph holds no sensor readings.'`, and the maintenance-history clause in the same string is never reached. Both full-routing trials reported a third of the answer as unanswerable. **This is not the dual-database design working.** Refusing a reading question is correct; handing `cypher_node` a reading question when maintenance history was wanted is not. Pair 4 answers that exact sub-question well in 17.6 seconds when it is asked alone.
    - **A data defect upstream of the agent explains the 0.33 routes.** `sensors.unit` holds `°C` with the degree sign. Genie sometimes emits `unit = 'C'` or `LOWER(unit) = 'c'`, matches zero rows, and the agent then states flatly that no engine shows abnormal EGT. That is false: the trials with no unit filter found 4,980-plus abnormal readings on CFM56-7B #1 of N10000. Zero rows makes the supervisor synthesize after one tool, so the Genie defect causes the routing symptom rather than the other way round.
    - **`SUPERVISOR_PROMPT` rule 1 says "call each tool at most once" and nothing enforces it.** `cypher_node` ran twice in both full-routing trials, returning the identical refusal each time.
    - **The worst single observation is not a routing miss, and it is not confined to Lab 5.** In the trial where Genie returned zero rows and `graphrag_node` never ran, Genie supplied its own manual-sounding prose and the synthesizer quoted it as maintenance-manual guidance, sourced to `genie_node`. It came from no manual. Plausible, generic, retrieved from nothing, and carrying a provenance line.
      - **The root cause is one defect in how sourcing is derived.** `_record` at `tools.py:595` stamps `{"tool": tool, "content": content}` unconditionally: the label is the name of the node that emitted the text, with no reference to where the text came from. `_format_findings` at `tools.py:586` renders it as `### {tool}`, and `tools.py:551` tells the synthesizer "The findings are the only source." **The attribution cannot be wrong because it never makes a claim that could be false, which is exactly what makes it useless as a check.**
      - **It surfaces in Lab 6 as well, by the same mechanism.** A rewrite built from a polluted memory still gets a clean sources line. Framing this as two mechanisms with one signature invites two fixes; it is one defect in two places, and that is where a fix belongs. **The reframing is the other session's, not mine.**
    - **Neither briefed risk was a factor.** `gold.sensor_health` still carries `health_status` on the deployed table, but no pair reads it; Genie used `sensor_readings`, `sensors`, `systems` and `aircraft` on every trial. The leftover Lab 6 memory graph produced no observable effect, and version 12 contains no memory code, so no memory claim is being made either way.
    - **Pair 4's Cypher carries `LIMIT 25` and returned 23 rows.** Two more maintenance events on N10004 and that answer truncates silently.
...
- **`f024ea61` was found empty a second time on 2026-08-09, and `471c14c2` was emptied the same day.** The routing suite could not use `f024ea61` and ran on `471c14c2` before it went.
  - **Cause: Ryan cleared them by hand. Closed, no defect.** Recorded because three sessions spent real time treating it as an unexplained fault, and the next person to find an empty instance should read this line before opening an investigation.
  - **The investigation did leave one useful negative.** Nothing in this repository can produce the shape that was observed on `f024ea61`, fleet labels gone with the memory schema intact. `clear_database` at `populate_aircraft_db/loader.py:525` is an unlabelled `MATCH (n) ... DETACH DELETE n` loop, so `clean` takes memory nodes too. `clear_enrichment_data` at `pipeline.py:713` never touches `Aircraft` or `Reading`. The only label-scoped delete loop in the tree, `profile_lab2_load.py:99`, is called with one label, `Flight`.
  - **The operational rule survives the closure, for a different reason.** Check `MATCH (n) RETURN count(n)` before trusting any measurement taken on a shared instance. Three sessions and a hand-clearing user share these three instances, so an instance being loaded is a fact with a short shelf life whoever emptied it.
  - **Reloaded again 2026-08-09 with `setup --skip-extraction`.** 21,547 operational nodes (Flight 14,543, Delay 5,541, Component 612, Sensor 288, MaintenanceEvent 286, System 144, Removal 57, Airport 40, Aircraft 36), 286 chunks all embedded at 1024 dims through `databricks-bge-large-en`, 5 Documents, 20 OperatingLimits, 52,138 relationships, both GraphRAG indexes created. That is the footprint after the halving, against the 177k of the previous reload.
- **`setup` does not clear the database, and a previous line here implied it did.** Measured on the 2026-08-09 reload: the run went straight from constraints to MERGE loads, and the only delete was `clear_enrichment_data` at 212 nodes. **The Lab 6 memory graph survived it** — `HAS_CONVERSATION` 7, `HAS_MESSAGE` 18, `NEXT_MESSAGE` 11, `MENTIONS` 18, all still present afterwards. It is `clean` that is unlabelled and total, not `setup`. Conflating the two makes `setup` look destructive and makes `clean` look survivable, and the second half of that mistake is the dangerous one.
- **A freshly loaded instance has no `Reading` nodes, and that is the design, not a gap.** Two sessions read it as a symptom on 2026-08-09 before checking. The loader has no `Reading` step at all: nine node types go into Neo4j and telemetry is not one of them. `nodes_readings.csv` is 10.2 MB and feeds the lakehouse instead, through `dlt_fleet_etl.py:114` into `bronze_readings` and on to `gold.sensor_readings` at 155,520 rows. So `cypher_node` answering "the graph holds no sensor readings" is correct behaviour. **The real routing defect to look for is the opposite one:** a lakehouse question sent to `cypher_node` rather than to Genie. Score those two apart.
