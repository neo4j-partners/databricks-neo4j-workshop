...
After notebook 01 completes, Neo4j holds the full Aircraft Digital Twin graph: the fleet with its systems, components, and sensors, plus flights, delays, maintenance events, removals, and the 20 canonical `OperatingLimit` nodes. The sensor readings stay in Databricks Delta tables. For the full schema reference, including exact row counts, see [DATA_GENERATOR.md](../workshop-setup/populate_aircraft_db/DATA_GENERATOR.md).
...
### Duplicate nodes, or a constraint violation on re-run

- The notebook writes nodes in **Append** mode, which is a plain `CREATE`. It
  never replaces a node. That is only safe because Section 4 wipes the
  database first, under `CLEAR_DATABASE = True` from Section 1.
- So re-running the whole notebook from the top is the supported way to
  reload. Re-running a load cell on its own is not: the uniqueness constraints
  Section 4 creates will reject the second copy and the cell fails.
- If you get there anyway, clear the database and rerun the notebook in order:
  ```cypher
...
- Continue to [Lab 4 - Compound AI Agents](../Lab_4_Compound_AI_Agents) to build a Genie space over the lakehouse telemetry in Part A. Part B, which adds a Supervisor Agent over Neo4j MCP, is a demo the instructor runs
