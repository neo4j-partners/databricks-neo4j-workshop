...
2. In the **Connect your data** dialog, select `databricks-neo4j-workshop` > `lakehouse` > then select `sensor_readings`, `aircraft`, `sensors`, `systems`
...
**"SQL query failed"** -- Verify table names in Unity Catalog: `databricks-neo4j-workshop.lakehouse`. Check column names match documentation. Ensure Genie space has access to all required tables. Test queries directly in SQL Editor first.
