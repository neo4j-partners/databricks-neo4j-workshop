...
The workshop builds toward a **LangGraph supervisor agent** in Lab 5 that routes questions across three tools: a Genie Agent for sensor analytics in SQL, Cypher over the participant's own Aura instance, and GraphRAG retrieval over maintenance manuals. Lab 6 then gives that agent memory in Neo4j. Lab 4 Part B is an instructor demo, not a participant exercise: the instructor builds a no-code Supervisor Agent in Databricks Agent Bricks over Neo4j MCP against their own demo instance, and participants watch.
...
ETL` pipeline and the comments a Genie Agent reads. Vocareum's
