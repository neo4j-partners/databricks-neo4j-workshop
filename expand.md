...
- **Memory spike done, 2026-08-08, against `1a2c98cc`. Verdict GO with two hard conditions.** The headline demo works and is compelling, Databricks Foundation Model endpoints satisfy the library's provider Protocols with no library change, and the install is pinned to a commit sha on the `mentions` branch of the `neo4j-partners` fork, which is released 0.5.0 plus the `MENTIONS` fix. The two conditions are install from that pinned sha with the `[nams]` extra, which is where `httpx` comes from, and adopt `Aircraft` only. Writing with `extraction_mode="explicit"` stays the recommended write mode. Each is a Phase 3 checklist item below, and each is a silent failure rather than a loud one. Full report in `worklog/memory-spike.md`.
...
Use [`neo4j-agent-memory`](https://github.com/neo4j-labs/agent-memory), installed from the `mentions` branch of the [`neo4j-partners` fork](https://github.com/neo4j-partners/agent-memory) and pinned to a commit sha, on the self-hosted bolt path. `client.schema.adopt_existing_graph(...)` adopts the fleet graph as long-term memory, so remembered entities resolve to the real `Aircraft` nodes from Lab 2 rather than creating parallel copies. Add `recall` and `remember` nodes on either side of the Lab 5 supervisor.
...
| Install `neo4j-agent-memory[nams]` from a commit sha on the `mentions` branch of the `neo4j-partners` fork, adopt `Aircraft` only, and write with `extraction_mode="explicit"` | The two hard conditions the Phase 0 spike attached to its GO, plus the write mode it recommends. Each was measured, and each fails silently rather than loudly, so none of them can be left to a default. The `[nams]` extra is the only place `httpx` ships, so it replaces the separate httpx condition | Floating the version, pinning the branch name rather than a sha, installing released 0.5.0 from PyPI, adopting every fleet label, relying on automatic LLM entity extraction |
...
- **Exact version.** Not PyPI, because released 0.5.0 silently drops `MENTIONS` edges. The notebook installs a commit sha on the `mentions` branch of the fork https://github.com/neo4j-partners/agent-memory, with the placeholder below replaced by that sha:

  ```
  %pip install "neo4j-agent-memory[nams] @ git+https://github.com/neo4j-partners/agent-memory.git@SHA_PLACEHOLDER"
  dbutils.library.restartPython()
  ```

  Pin the sha, never the branch name `@mentions`, because a branch moves and that is the exact failure mode the original `==0.5.0` pin existed to prevent. The `[nams]` extra contains exactly one thing, `httpx>=0.27.0` at `pyproject.toml:111`, so it replaces the separate httpx install. **The branch bumps `pyproject.toml` to `0.5.1.dev0+mentions`**, so `pip list` tells a patched install from an unpatched one at a glance. The cost of that local version segment is that it does not resolve from PyPI, so anything pinning by version alone, MLflow's inferred requirements above all, has to be handed the wheel or the git URL explicitly. The base is released 0.5.0: wheel uploaded 2026-05-30T23:23:11, upstream repository https://github.com/neo4j-labs/agent-memory, tag `python-v0.5.0`, source commit `ece2e6ee1c594359381a7066ac05b2219ebf9cfb`, 2026-05-30, William Lyon. `requires-python >=3.10`.
...
- [ ] **Install `neo4j-agent-memory[nams]` from a pinned commit sha on the fork, not from PyPI.** The notebook line, with the placeholder below replaced by the commit sha:
...
- [ ] **Make the patched package installable from Model Serving.** A `git+https` dependency has to resolve inside the serving container, which is not something to assume. The robust path is `uv build` on the `mentions` branch, the wheel uploaded to a Unity Catalog volume, and installed from the volume path in both the notebook and the logged model's dependencies. This applies to the Labs 5 and 6 endpoint once the memory nodes are on it
