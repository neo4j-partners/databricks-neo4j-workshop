...
== What You Build, and the Optional Demo Beside It

* **The lab is the Genie Agent.** You build it over the Lakehouse telemetry, attach four gold tables to it, and ask it questions. Open `Lab_4_Compound_AI_Agents/04_genie_agent.ipynb`; every step lives there. It produces a Genie Agent ID that Lab 5 and Lab 6 both consume.
* **The compound AI agent is optional.** A no-code Supervisor Agent in Agent Bricks puts the Genie Agent and a Neo4j MCP agent behind one question box. It is optional because it needs a hosted MCP server and the Unity Catalog connection that fronts it, both outside the scope of the workshop. The instructor demos it. The procedure is in `Lab_4_Compound_AI_Agents/OPTIONAL_COMPOUND_AGENT_DEMO.md`.

**Neither one is the workshop's final architecture.** Lab 5 is, and it reaches the same routing behaviour in code against your own graph, with no MCP server needed. The demo exists to show what the no-code path looks like and what it costs, which is a contrast worth seeing before you write the code version.
...
* **Supervisor Agent** (Agent Bricks) routes questions to the right specialist: SQL questions to Genie, graph questions to Neo4j, combined questions to both. **Optional demo, and Lab 5 rebuilds this in code**
...
image::lab-architecture-overview.svg[An Agent Bricks supervisor routing to Genie and a Neo4j MCP agent]
...
NOTE: The diagram above is the **optional demo**, not the workshop's destination. You build the Genie Agent. The supervisor and its Neo4j MCP agent sit outside the scope of the workshop, because they need a hosted MCP server and the Unity Catalog connection that fronts it. The instructor demos them. Lab 5 is where you build this routing yourself, in code, against your own graph.

=== The demo and Lab 5, side by side

The same routing behaviour reached two ways. This contrast is the reason the demo is worth watching rather than skipping.
...
Neither column is the right answer in general. Agent Bricks is faster to stand up and easier to hand to a non-engineer; Lab 5 is what you reach for when the routing rules themselves need testing, review and a git history.
...
Then continue to Lab 5, where the supervisor becomes code you own. If the instructor runs the optional compound agent demo, watch it first.
