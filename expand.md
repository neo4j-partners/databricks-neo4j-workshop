...
>
> **Lab 4 no longer has parts, as of 2026-08-09.** The lab is the Genie Agent, and the compound agent over Neo4j MCP is an optional demo in [`Lab_4_Compound_AI_Agents/OPTIONAL_COMPOUND_AGENT_DEMO.md`](Lab_4_Compound_AI_Agents/OPTIONAL_COMPOUND_AGENT_DEMO.md), which is `PART_B.md` renamed. Read "Part A" below as the lab itself and "Part B" as that optional demo. Path references were updated to the new name; the prose was left as written.
...
| Track | Where it is |
|---|---|
| **A. Engineering** | Phase 0 loader work **done and verified against a live Aura instance.** The Lab 5 catch-up cell it was built for is **dropped as too complex, 2026-08-08**; the loader changes are kept and are load-bearing elsewhere. Phase 1 core agent **built and measured against a rebuilt participant-shaped Aura instance: 38 cells, 0 errors, all three tools live, routing 12 of 12.** Phase 2, deployment, is next |
...
Eleven items are being worked right now, in parallel. None of them is verified, and none should be read as measured.

- **The Lab 5 catch-up cell is dropped, as too complex.** It was never written, so nothing is being unbuilt. The loader work done for it stays: the `--skip-extraction` flag, the `databricks` embedding provider, and the checked-in `OperatingLimit` CSV are all load-bearing for Labs 2, 3 and 5 independently of any catch-up path, and reverting them would break the graph. The Lab 6 catch-up cell planned for Phase 5 goes with it. See The Cost section for what the complexity actually was, and Open Decision 10, which this drop reopens as the only remaining recovery path for a participant whose instance is broken.
...
Leave both parts where they are. `Lab_4_Compound_AI_Agents/` keeps its name, its README, `PART_A.md`, and `OPTIONAL_COMPOUND_AGENT_DEMO.md`. **Decided 2026-08-08, in flight.** The section below was written when Part B was optional-but-buildable; the edits it describes are superseded by Open Decision 9.
...
Under the original structure, Lab 4 Part B was a safety net. A participant who never finished Lab 2 still got a working agent, because the agent queried somebody else's fully loaded database. **That safety net is gone as of the 2026-08-08 instructor-demo decision**, since a participant no longer runs Part B at all. Labs 2 and 3 are load-bearing for Labs 5 and 6 with nothing behind them.

**~~A catch-up cell at the top of Lab 5.~~ Dropped 2026-08-08, because it is too complex.**

The idea was one idempotent cell that brings a participant's Aura instance to the state Labs 2 and 3 would have left it in, so anyone behind runs it and continues. This document called it the single most important item in the whole proposal. It is dropped anyway, and complexity is the whole reason.

What the cell actually required, once the loader work was done and the cell itself was the only thing left: `populate_aircraft_db` does not run on serverless, so its loader module had to be vendored into a notebook cell and then kept in step with the package it was copied from. That cell then needed the participant's credentials, the Unity Catalog volume, the embedding endpoint, and an idempotent write path, and it had to land a schema that Lab 2's Spark Connector path and Lab 3's notebook path also produce, leaving three code paths that must agree forever. Phase 5 then wanted a second one at the top of Lab 6 reproducing end-of-Lab-5 state, agent included. That is a lot of machinery whose only job is to let somebody skip the labs the workshop is about.

**What now covers the failure it covered.** A participant behind at Lab 5 re-runs Lab 2 and Lab 3 notebook 01, which is what the catch-up cell was a shortcut for rather than a substitute for. A participant with no working instance at all makes a new AuraDB Free instance and does the same. **The cost is honest and should be stated to instructors:** somebody who is an hour behind at 2pm does not catch up in ten minutes any more, and Labs 5 and 6 are lost for them. See Open Decision 10 for the fallback that now carries more weight than it did.

**The loader work behind the cell stays, and must not be reverted.** All three changes are load-bearing elsewhere and none of them depend on the cell existing.

Reuse of `workshop-setup/populate_aircraft_db` was the right call for the loader itself. `uv run populate-aircraft-db setup` loads the CSVs, chunks the manual, generates embeddings, and creates the indexes, and `loader.py` checks for the index named `maintenanceChunkEmbeddings`, which is the same name Lab 3 notebook 01 creates. The tool targets Lab 3's schema. Rebuilding would have meant reproducing that schema by hand and maintaining two loaders that must agree forever.

Two changes were made to it. **Both are done and verified, and both are kept.**
...
**Add a flag to skip entity extraction.** The `setup` command also runs `SimpleKGPipeline` entity extraction, which needs an LLM API key and is not something Lab 5 depends on. The skip path needs CSVs, chunks, embeddings, and indexes. Nothing else. It is kept because it is the only way to load a Lab 5-shaped graph without an LLM key, which is what every development and dry-run load has used since.

This was written as a small change and it was not one. `SimpleKGPipeline` builds chunking, embedding, and extraction as one object with no seam between them, and the index creation the skip path exists for runs after it, so a flag that wraps the call would also skip the index. What shipped instead assembles the library's own components minus the extractor, and puts the branch inside the enrich step so index creation and cross-linking still run on both paths. Everything else in Lab 3, and all three Lab 5 tools, work against the resulting graph.

Skipping the extractor did leave one gap, and it is being closed at the source rather than documented as a limitation. No extractor meant no `OperatingLimit` nodes, so Lab 3 notebook 02's `limit_retriever` cell had nothing to return on a skip-extraction graph. The fix: the operating limits are extracted from the five maintenance manuals once, into a checked-in CSV, and the loader writes `OperatingLimit` nodes and `HAS_LIMIT` edges from it like any other node CSV. Deterministic, no LLM, no key. The skip path then produces the same graph the extraction path does for this entity type. **This is the change with the least to do with the dropped cell and the most to do with everything else.** That CSV is now what Lab 2 loads and what Open Decision 8 means by `OperatingLimit`, so it is load-bearing for Labs 2, 3 and 5 whether or not a catch-up path ever existed. **Done and verified on `f024ea61`:** `OperatingLimit` 20, matching the CSV row count, `HAS_LIMIT` 288, one per sensor across 36 aircraft, zero differences across all 44 metrics between two consecutive full runs, `verify` clean on the skip path, and the `limit_retriever` cell returning populated results when run with the notebook's own query and question.
...
**Dropping the catch-up cell raised this item's value rather than lowering it.** It was written as the second of three mitigations, behind a catch-up cell that no longer exists. It is now the only thing standing between a participant with a broken instance and losing Labs 5 and 6. Open Decision 10 was worth deferring when it was insurance on top of a working recovery path. It is worth settling now.
...
The split is not free. A half-day advanced session is a different room on a different day, and Aura Free instances pause after three days of inactivity and are deleted after 30. Whatever a participant loaded in the foundation session may be gone, paused, or forgotten by the time the advanced session starts, and there is now no catch-up path at all. **The 2026-08-08 catch-up drop makes this worse, and it should be read as the split's largest open problem rather than as a detail.** A returning participant whose instance paused, was deleted, or whose credentials went with the laptop they left at the office has to re-run Labs 2 and 3 before Lab 6 starts. Phase 5 was going to solve this with a second catch-up cell; that cell is dropped, so what Phase 5 owes is written guidance rather than code. Until then, the full day is the supported format and the split is best effort.
...
- `Lab_4_Compound_AI_Agents/OPTIONAL_COMPOUND_AGENT_DEMO.md`: add an instructor-demo banner at the top. No content changes, because the procedure is what the instructor follows.
...
- `workshop-setup/populate_aircraft_db/src/populate_aircraft_db/main.py`: add a flag to skip entity extraction. Built for the catch-up path, kept after that path was dropped, because it is the only way to load a Lab 5-shaped graph without an LLM key.
- `workshop-setup/populate_aircraft_db`: a checked-in CSV of operating limits extracted from the five maintenance manuals, plus loader code that writes `OperatingLimit` nodes and `HAS_LIMIT` edges from it like any other node CSV. This takes the one entity type Lab 3 notebook 02 queries directly off the LLM extraction path, so `limit_retriever` returns results on a skip-extraction graph. Kept and now load-bearing: this CSV is what Lab 2 loads and what Open Decision 8 means by `OperatingLimit`.
- `workshop-setup/aircraft_digital_twin_data/nodes_sensors.csv` and `workshop-setup/populate_aircraft_db/src/populate_aircraft_db/generator/fleet.py`: `Sensor.unit` normalized to the maintenance manual limit strings, `°C` and `% RPM`, so `Sensor.unit` and `OperatingLimit.unit` join. Done and verified. The unit strings that appear in `Lab_4_Compound_AI_Agents/PART_A.md`, `Lab_4_Compound_AI_Agents/OPTIONAL_COMPOUND_AGENT_DEMO.md`, `Lab_2_Databricks_ETL_Neo4j/data-exploring.md`, and `workshop-setup/docs/MANUAL_SETUP.md` were updated in the same pass.
...
- `slides/`: ~~`slides/platform-overview/01-workshop-over.md:126-143` still lists the MCP infrastructure as required shared provisioning, and the slides still describe the old shared-versus-personal Aura split. **Deferred to Phase 5** for the same reason as the site, and rewritten in the same pass.~~ **Done, verified 2026-08-09.** The file is now `slides/platform-overview/01-workshop-overview-slides.md` and the section is `:119-143`. It reads the other way: the MCP server and its Unity Catalog connection are both marked "used only in the Part B demo" and the demo instance says "Participants never connect to it", `:141` states "Every required lab uses your own instance", and `:143` marks Lab 4 Part B an instructor demo needing no credential and no setup.
...
10. **Where the broken-instance fallback points, now that the reference instance is the instructor's demo instance.** Open Decision 9 removed the participant from that database, and the loader still treats it as its default write target. The mitigation in The Cost section still describes handing out its credentials as a three-variable override, and the Phase 4 checklist still has an item verifying that override. The failure it covers is real: a participant whose own Aura is broken or expired at 2pm. The target is not.

   **Reframed 2026-08-08 by the catch-up drop, which removed one of the two options.** "Lean entirely on the Lab 5 catch-up cell" is void, because there is no catch-up cell. What is left:

   - **A second Aura instance, loaded and left read-only**, never the loader's write target, credentials handed out only on request. Costs one instance to keep loaded and one paragraph of documentation.
   - **Drop the fallback and lean on AuraDB Free being free and instant.** A participant with a dead instance makes a new one and re-runs Lab 2 and Lab 3 notebook 01. No shared credential, nothing to keep loaded, nothing a `clean` can wipe. Costs the participant roughly the length of Lab 2 plus Lab 3 notebook 01, which at 2pm may mean they do not finish.

   Before the catch-up drop the second option was clearly right and I recommended it. It is now the option that leaves a broken instance with no recovery inside the session. **Not decided, and it is the one open item that decides whether a participant who falls behind has any path back.** The related fix either way: `Lab_1_Aura_Setup/Aura_Free_Trial.md` still says to click **Start 14-day free trial**, which provisions the Professional tier that expires mid-course and is the single largest cause of the failure this decision covers.
...
- The Unity Catalog volume holds everything the loader needs, including the maintenance manual that Lab 3 notebook 01 chunks.
...
| ~~Reuse `populate_aircraft_db` as the catch-up loader~~ **Amended 2026-08-08: the catch-up cell is dropped, the loader reuse stands** | It already produces Lab 3's schema and index names, so one code path stays correct instead of two agreeing by luck. That reasoning was about the loader and survives the cell being dropped: the tool is still the admin and dry-run load path | Writing a fresh Spark Connector loader |
| **The Lab 5 catch-up cell is dropped, as too complex** | Vendoring a non-serverless loader module into a notebook cell, keeping it in step with the package it was copied from, and leaving three code paths that must produce one schema, all so a participant can skip the labs the workshop teaches. Phase 5 wanted a second one for Lab 6. See The Cost section | Shipping it anyway and carrying the third code path; a cut-down cell that loads only the fleet graph and leaves the Lab 3 artifacts missing |
...
- [x] Write `OperatingLimit` nodes and `HAS_LIMIT` edges deterministically. Shipped as a checked-in CSV the loader writes like any other node CSV. No LLM and no API key on the skip path. Measured on `f024ea61`: `OperatingLimit` 20, matching the CSV row count exactly, and `HAS_LIMIT` 288, one per sensor across 36 aircraft. Two consecutive full runs differ on zero of 44 metrics, and `verify` reports zero warnings on the skip path
...
- [ ] **Add a refusal rule to the `cypher_node` instructions: never substitute a limit, threshold, or ceiling for a measurement.** Asked for the highest average vibration, the tool returned 3.0, which is `OperatingLimit.maxValue` for `Vibration - B737-800`, against Genie's measured 0.3646 ips, and the supervisor printed both as rival answers. The graph holds no readings, so the honest answer was to decline. **Landed in `tools.py` and described in the notebook, and unmeasured.** The 12-of-12 run predates it, and the change edits the prompt both the supervisor and the Cypher tool read, so the routing numbers above are no longer numbers for the code on disk. One re-run closes it
...
- [x] `graphrag_node` degrades to a clear message rather than failing at import when the vector index is absent. **Done, verified on disk 2026-08-08:** `graphrag_node_unavailable` plus an `available_tools` parameter the supervisor reads, so a missing index drops the tool from the routing list instead of raising
- [ ] Optional hybrid retrieval exercise for participants who ran Lab 3 notebook 03. **The one remaining item marked optional; cut it if Phase 2 is the priority**
- [x] Credentials read from the `fleet-ops-<user-slug>` secret scope Lab 3 notebook 01 created. No plaintext password anywhere in Lab 5. **Done, verified on disk 2026-08-08:** `secret_scope_name(spark)` names the scope and `open_driver_from_secrets(dbutils, SECRET_SCOPE)` reads, uses and drops the password inside the call
- [x] A scope-creation block, **commented out by default**, above a note saying to run it only if Lab 3 was skipped. It is the recovery path, not the normal path. **Done, verified on disk 2026-08-08**
...
Completion criteria: the anchor question about abnormal EGT, maintenance history, and the relevant manual procedure returns a correct answer in-notebook, and each of the four routing cases lands on the expected tool. Report `cypher_node` versus `graphrag_node` routing accuracy as its own number rather than folding it into an overall score, because `VectorCypherRetriever` makes those two tools adjacent and that pair is where misrouting will show up first. **Met 2026-08-08: anchor question correct end to end, 12 of 12 overall, 8 of 8 on the adjacent pair.**

**What is left in this phase, as of the 2026-08-08 review.** Credential wiring, the degradation path and the scope-creation block are done and were verified on disk; this checklist was behind the code. Three items remain, and two of them close in a single run:

1. **Re-measure with the refusal rule in place.** It changes the prompt the earlier numbers were taken against.
2. **Exercise the extracted-entity path.** Needs a graph loaded with extraction on, so it is blocked on the `ExtractedLimit` split landing. Same run as item 1.
3. **The hybrid retrieval exercise**, optional, and cuttable.

The re-run is one pass and should wait for the label split so both measurements come from one graph.
...
- [x] ~~Catch-up loader exercised from a deliberately incomplete Lab 2 state~~ **Retired 2026-08-08 with the catch-up cell.** The dry run above covers the loader on the path that still exists, which is a full load from empty
...
- [x] ~~**Rewrite the slides in `slides/`.** `slides/platform-overview/01-workshop-over.md:126-143` still lists the MCP infrastructure as required shared provisioning, and the slides still describe the old shared-versus-personal Aura split. Same pass as the site, same reason for waiting~~ **Done, verified 2026-08-09.** The deck is now `slides/platform-overview/01-workshop-overview-slides.md`, renumbered folder-local when four files were found sharing the `01-` prefix, and the section is `:119-143`. Both claims are inverted there: the MCP server and its Unity Catalog connection read "used only in the Part B demo", the demo instance reads "Participants never connect to it", `:141` says "Every required lab uses your own instance" and names Labs 2, 3, 5 and 6, and `:143` says Part B needs no credential, no MCP connection and no setup
- [x] ~~Catch-up cell at the top of Lab 6 that brings a returning participant to end-of-Lab-5 state: fleet graph, Lab 3 artifacts, and a working Lab 5 agent. Reuses the Lab 5 catch-up cell and adds the agent~~ **Dropped 2026-08-08 with the Lab 5 catch-up cell it was built on.** A returning participant re-runs the labs. This item was the second cell and the harder one, since it had to reproduce a working agent as well as a graph, and it is the clearest single reason the approach was too complex to keep
...
|---|---|---|---|---|---|---|
| **A. Engineering** | Loader work **(done)**; limit label split, constraint key, and the `neo4j_database` fix **(in flight)** | Three tools plus supervisor **(built and measured)**; refusal rule **(landed, unmeasured)**; credential wiring, degradation path **(done)** | Deploy and evaluate | Memory nodes and demos | Dry run | ~~Lab 6 catch-up cell~~ **(dropped)** |
...
- The loader does not block building the Lab 5 tools. Build the tools against an already-loaded instance. **With the catch-up cell dropped, the loader is no longer a participant-facing prerequisite at all**, only the admin and dry-run load path.
...
- ~~Phase 5 depends on Phase 4, not on Phase 3. Its catch-up cell has to reproduce a state Phase 4 has confirmed is reachable, so building it earlier means building it against a moving target.~~ **Void 2026-08-08**: the cell that created the dependency is dropped, so what remains in Phase 5 is documentation and the between-sessions guidance.
...
- ~~`populate_aircraft_db` will not run on serverless: vendor its loader module into the catch-up notebook.~~ **This risk did not get mitigated, it got avoided.** Vendoring the loader module into a notebook was the largest single piece of the catch-up cell's complexity and the main reason the cell is dropped. Nothing participant-facing runs the loader now, so where it runs stopped mattering.
