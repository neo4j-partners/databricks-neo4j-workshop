...
* **Neo4j MCP Agent** translates natural language to Cypher for graph traversal via MCP tools (get_neo4j_schema, read_neo4j_cypher)
...
NOTE: You build the Genie space in Part A. The supervisor and its Neo4j MCP agent are Part B, which the instructor demonstrates against their own demo instance and MCP connection. Watching it requires no Aura instance, no MCP connection, and no OAuth credential from you. Lab 5 is where you build this routing yourself, in code, against your own graph.
