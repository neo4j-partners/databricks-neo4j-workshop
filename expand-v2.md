...
- **Does the Genie space get four tables or six?** `lab/workshop.py:143-144` puts `fleet_readiness` and `sensor_health` in `GOLD_TABLES`, `:374-375` writes table comments for both, and `:392-400` writes column comments for `readiness_status` and `health_status`. Those comments exist so a Genie space answers correctly. But `PART_A.md:40` tells the participant to select four tables, `sensor_readings`, `aircraft`, `sensors` and `systems`, and never mentions either summary table. **So provisioning comments two tables the Genie space is never pointed at.** Either `PART_A.md` is missing two tables or `workshop.py` comments tables Lab 4 does not use. The Lab 4 notebook follows `PART_A.md` and selects four, so it is consistent with the lab as written. Guessing changes what Genie can answer, so this is a content call.
