...
**Duration:** ~30 minutes for Part A, which you build. Part B is an instructor demo, about 20 minutes on screen.

== Prerequisites

* Running in a **Databricks workspace** with Agent Bricks access

Part B needs nothing from you. The instructor supplies the demo workspace, the Unity Catalog MCP connection, and the graph behind it.

**Recommended:** Complete **Lab 2**, the Databricks ETL lab, before starting this one. Part A uses shared Lakehouse tables rather than your personal Aura instance, but Lab 2 gives you familiarity with the data model that the agents here query: the aircraft topology, sensor relationships, flights, and maintenance events.

NOTE: Part A uses **shared** Lakehouse tables in Unity Catalog, so you do not need data in your personal Aura instance for it.

== Part A: Genie Space for Sensor Analytics

Create an AI/BI Genie space that enables natural language queries over sensor telemetry. This is the part you build.
...
- EGT (Exhaust Gas Temperature): measured in C. The normal range is per model, taken from each maintenance manual's takeoff limits: A320-200 620-680, A220-300 855-890, E190 870-900, B737-800 900-950, A321neo 980-1040. Always filter or group by model before comparing EGT across aircraft.
...
- FuelFlow: measured in kg/s. The normal range is per model: E190 1.00-1.20, A220-300 1.15-1.35, B737-800 1.20-1.50, A320-200 1.20-1.95, A321neo 1.50-2.00.
...
Click **Save** to preserve your Genie space configuration. Note the exact name of your Genie space, for example `Aircraft Sensor Analyst RK`. Lab 5 uses it as one of its three tools.

== Part B: Supervisor Agent (Instructor Demo)

IMPORTANT: Watch this part. Do not build it. The instructor runs the demo from the front of the room against their own demo Aura instance and their own Unity Catalog MCP connection. You need no Aura instance, no MCP connection, and no OAuth credential to follow along. Your continuation from Part A is Lab 5.

The demo shows the same routing architecture as Lab 5, built with no code and with centrally governed access to Neo4j through Unity Catalog. That contrast is the reason it is in the workshop.

The steps below are the record of how the demo is built. They are written for the instructor running it.

=== Verify Neo4j MCP Connection

Confirm the connection is present in the demo workspace before the session.
...
Verify the MCP tools work before building the supervisor:
...
3. Select the demo Genie space built as in Part A
...
SENSOR TYPES:
- EGT: Exhaust Gas Temperature, per model: A320-200 620-680 C, A220-300 855-890 C, E190 870-900 C, B737-800 900-950 C, A321neo 980-1040 C
- Vibration: Engine vibration (0.05-0.50 ips)
- N1Speed: Fan speed (85-100, unit column value % RPM)
- FuelFlow: Fuel consumption, per model: E190 1.00-1.20 kg/s, A220-300 1.15-1.35 kg/s, B737-800 1.20-1.50 kg/s, A320-200 1.20-1.95 kg/s, A321neo 1.50-2.00 kg/s
...
**"Cypher query failed"** -- Check that the demo Aura instance was loaded correctly with `workshop-setup/populate_aircraft_db`. Verify node labels and relationship types match documentation. Review Cypher syntax for errors. Test queries directly in Neo4j Aura console first.
...
Your Genie space from Part A is the first of three tools in Lab 5, where you build the same routing in code, against your own graph, and deploy it to Model Serving.
