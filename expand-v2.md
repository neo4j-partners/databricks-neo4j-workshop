...
- **MEASURED: dropping a Delta table destroys the grants MLflow issued for it, and that is a second independent cause of the Genie failure.** `SHOW GRANTS ON TABLE databricks-neo4j-workshop.aircraft.sensor_readings` returned one row, `account users | SELECT`, and nothing for the endpoint's service principal. Query history shows `DROP TABLE` on `sensors` and `sensor_readings` at `01:23Z`, followed by the rebuild recorded under "Lakehouse gold tables". **A grant does not survive the table it was written against.** Any endpoint logged before that rebuild therefore lost table access even with a correct `resources` list, so the missing warehouse resource was never the whole story. The eight `DatabricksTable` entries are what let a redeploy reissue the grants. **The operational rule: rebuilding a gold table obsoletes every endpoint logged before it, and the repair is a redeploy, not a regrant.**
...
- **The drop also invalidated the deployed endpoint, which was not obvious at the time.** Dropping the four tables took the endpoint's table grants with them. See "Lab 5 deployment" for the measurement and the rule that follows from it.
...
- **Cell 19 of notebook 02 prints `Usage: {}` for every participant.** The endpoint returns an empty usage block and the cell prints it anyway. One line to fix: drop `print("Usage:", result["usage"])`, or say in the markdown above it that Model Serving returns no token counts for this endpoint and latency is the only cost signal available. Cheap, and it is the first thing a participant sees after the deploy they just waited fifteen minutes for.
...
Fifteen, numbered. Each says what the decision is, what the evidence is, and what it blocks. **Five closed during this pass**, questions 1 through 4 and question 14, and each is struck through rather than deleted so the decision and its evidence stay readable. **Nothing is in flight.** The rest wait on a decision from Ryan rather than on work in progress.

**Numbers are stable across passes and the order is not.** Within each bucket the questions run most important first, so a number is an identity and a position is a judgement. **Read question 15 first**, then question 6.

### Closed this pass

1. ~~**Lab 5 has no notebook 02 in the participant path.**~~ **CLOSED, built and committed in `1be478f`.** `Lab_5_LangGraph_Agent/02_deploy_and_evaluate.ipynb`, 30 cells, 17 markdown and 13 code. It logs `agent.py` with the twelve-entry `build_resources`, pins `pip_requirements` rather than letting MLflow infer them, registers to Unity Catalog, deploys, waits for READY, and queries the endpoint. Lab 6 now has the prerequisite it lists. **The caveat that shipped with this question is now closed: the notebook has been run top to bottom and the committed file carries the outputs**, 13 code cells numbered 1 to 13, zero errors. See section 2, Lab 5 deployment.
...
3. **Provision a fresh AuraDB Free instance and run the index tolerance check**, reading the home database out of `SHOW DATABASES` rather than assuming `neo4j`. F5. **Still the only remaining no-go, still unrun**, and it touches nothing else, so it can run beside anything here. **The script this step named never existed.** An earlier pass recorded `scratchpad/aura_free_index_check.py` as written. There is no `scratchpad/` directory in the repository and nothing in git history ever created one; it was written into an ephemeral session scratchpad and is gone. **It has been rewritten with a durable home**: `workshop-setup/verify/src/verify_aura_caps/main.py`, run as `uv run verify-aura-caps`, with the two modes the lost version claimed, read-only `check` and apply-then-roll-back `probe`. It reads the home database out of `SHOW DATABASES` rather than assuming `neo4j`. **Only the run is still owed, and it needs the fresh Free instance. Ryan's to provision.**
