...
```sql
UNWIND ['ExtractedLimit'] AS label
CALL (label) {
...
> **Concepts**: `UNWIND` expands a list of labels into rows. The `CALL` subquery scopes a `MATCH` per label. `collect()` re-aggregates the results. These entities were extracted via SimpleKGPipeline during the enrich step. `ExtractedLimit` is extraction output only. Lab 2's 20 hand-transcribed limits carry the `OperatingLimit` label, which nothing in Lab 3 writes.
...
### Sensor to OperatingLimit, the canonical thresholds
...
```sql
MATCH (s:Sensor)-[:HAS_EXTRACTED_LIMIT]->(el:ExtractedLimit)
RETURN s.sensor_id AS source, el.name AS target
LIMIT 10
```

> **Concepts**: A parallel edge to a separate label. Same sensor, same parameter, a different source of the number. Keeping the two relationship types apart is what lets a query state which population it wants.
...
WHERE el.parameterName = ol.parameterName
RETURN DISTINCT ol.name AS Limit,
       ol.maxValue AS TranscribedMax,
       el.maxValue AS ExtractedMax,
       ol.unit AS Unit
ORDER BY Limit
```

> **Concepts**: The `WHERE` belongs to the `OPTIONAL MATCH`, so a limit with no extracted counterpart still returns a row with a null `ExtractedMax`. A null or a mismatch is a measurement of the extraction, not a failure of the query.

### Full provenance chain: ExtractedLimit to Chunk to Document to Aircraft
...
> **Concepts**: These cross-links connect the knowledge graph back to the operational aircraft graph. `FROM_CHUNK` exists on extracted entities alone, since a chunk is where the LLM read them; the canonical `OperatingLimit` rows came from a CSV and have no source chunk. The chain traces an extracted limit back through the chunk it was read from, that chunk's document, and the aircraft the document applies to.
